...
async function handleAppConfigLoad(event, state) {
    try {
        const file = event.target.files[0];
        if (!file) throw new Error('No appConfig.json file selected.');

        const fileContents = await readFileAsText(file);
        const appConfig = parseJson(fileContents);

        state.appId = appConfig.id || '';
        state.appUrl = appConfig.url || '';
        const appNameInput = document.getElementById('appName');
        const appDescriptionInput = document.getElementById('appDescription');
        if (appNameInput) appNameInput.value = appConfig.name || '';
        if (appDescriptionInput) appDescriptionInput.value = appConfig.description || '';

        resetComponentCounters(state, appConfig);
        populateAssignedComponents(state, appConfig);
...
async function handleUrlLoad(event, state, displayNodes) {
    try {
        ...
        const wfResponse = await fetch(`/saus/${encodeURIComponent(appUrl)}/wf.json${cacheBuster}`);
        if (!wfResponse.ok) {
            alert(`Failed to load wf.json from /saus/${appUrl}/wf.json`);
            return;
        }
        ...
        const appConfigResponse = await fetch(`/saus/${encodeURIComponent(appUrl)}/appConfig.json${cacheBuster}`);
        if (!appConfigResponse.ok) {
            alert(`Failed to load appConfig.json from /saus/${appUrl}/appConfig.json`);
            return;
        }
        ...
        await loadThumbnailFromappUrl(appUrl, state);

        
        window.parent.postMessage({ type: 'loadApp', appUrl: appUrl }, '*');

    } catch (error) {
        console.error('Error loading app from URL:', error);
        alert('Error loading app from URL. Please check the console for details.');
    }
}
...
function resetComponentCounters(state, appConfig) {
    state.componentCounters = {};

    for (const [componentTypePlural, components] of Object.entries(appConfig)) {
...
function populateAssignedComponents(state, appConfig) {
    ...
    if (Array.isArray(appConfig.multiComponents)) {
        appConfig.multiComponents.forEach(multiComponent => {
            const components = [];
...
function initializeFileHandlers(state, displayNodes) {
    ...
    if (!fileInput || !appConfigInput || !urlLoadButton || !thumbnailInputHidden || !thumbnailPreview) return;

    fileInput.addEventListener('change', event => handleFileUpload(event, state, displayNodes));
    appConfigInput.addEventListener('change', event => handleAppConfigLoad(event, state));
